...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-tsforecast-detail.ps1

node dist/tests/domainReview.test.js

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-tsforecast-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-tsforecast-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-tsforecast-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
